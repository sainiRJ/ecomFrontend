...
const ShopProduct = ({ product, addToCart }) => {
  return (
      ...
      <div className="product-item bg-light mb-4">
        <div className="product-img position-relative overflow-hidden" style={{ height: '300px'}}>
          ...
          <div className="product-action">
            <a className="btn btn-outline-dark btn-square" onClick={() => addToCart(product)}><i className="fa fa-shopping-cart"></i></a>
...
const ShopProducts = () => {
  ...
  const handleAddToCart = product => {
    ...
    axios.post('http://localhost:5001/product',product,{
      headers: {
  ...
  return (
    <div className="row px-xl-5">
      {products.map(product => <ShopProduct key={product.id} product={product} addToCart={handleAddToCart} />)}
    </div>
  );
};
